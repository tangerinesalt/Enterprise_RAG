...
The system SHALL serialize chat and config writes within the same session so that concurrent operations do not silently overwrite each other's updates.
...
- **WHEN** the system performs `new_chat`, `select_chat`, `delete(chat_file)`, `update_config`, `chat()`, or `chat_stream()` for the same session
...
#### Scenario: Preview update and active chat update share atomic config write
- **WHEN** the system updates `chat_previews` and `active_chat` for the same session
- **THEN** both changes are committed through the same atomic persistence path
- **THEN** the resulting `config.json` contains both fields in a single valid snapshot
