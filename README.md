...
- 知识库名、会话名和聊天文件名必须是安全标识符：不能为空，不能包含路径分隔符，不能是 `.` / `..`，不能带盘符或 UNC 前缀，不能以空格或点结尾，不能使用 Windows 保留设备名。
- Web 上传接口会先把客户端提交的文件名裁剪为叶子文件名，再写入知识库存储；裁剪后的叶子文件名如果不合法会直接拒绝。
- 知识库内的文件/文件夹目标和会话聊天文件读取/删除都会做根目录 containment 校验，拒绝 `../` 等越界访问。
- 新写入的助手消息统一持久化为 `content` + `additional_kwargs.sources`。旧的同步聊天记录仍可读取正文，但不会从正文里反解析来源面板。
